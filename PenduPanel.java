...
public class PenduPanel extends JPanel {
    ...
    SwingUtilities.invokeLater(() -> {
        if (motsPanel.isMotDevine()) {
            JOptionPane.showMessageDialog(this, "Bien joué ! T'es plus malin qu'un ogre qui a trouvé une fève magique !");
            resetJeu();
        } else if (imagePanel.getErreurs() >= 6) {
            JOptionPane.showMessageDialog(this, "Oh non ! À cause de toi, Shrek a fini par se retrouver suspendu ! Le mot était : " + motsPanel.getMotSecret());
            resetJeu();
        }
    });
}
